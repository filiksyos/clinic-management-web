...
const supabaseUrl = 'https://szcvvdedhulcrgpanudt.supabase.co';
const supabaseServiceRoleKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InN6Y3Z2ZGVkaHVsY3JncGFudWR0Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcwMDgyODU3OCwiZXhwIjoyMDE2NDA0NTc4fQ.wICFzK5Ssyx6KYFNK1Q9FeXsNLPyctTmW0VZeUjBPgE';
...
async function getAccessToken(): Promise<string> {
  ...
  try {
    ...
    // Process private key correctly for Deno
    // The private key needs special processing in Deno
    const privateKeyPEM = serviceAccount.private_key
      .replace(/\\n/g, '\n')
      .replace('-----BEGIN PRIVATE KEY-----\n', '')
      .replace('\n-----END PRIVATE KEY-----\n', '');
    
    console.log("Private key processed for Deno");
    
    // Base64 decode the PEM key to get the raw key data
    ...
    // Import the private key with explicit format
    try {
      ...
      console.error('Error importing key:', keyError);
      throw new Error(`Key import error: ${keyError instanceof Error ? keyError.message : String(keyError)}`);
    }
  } catch (error) {
...
serve(async (req: Request) => {
  ...
  try {
    ...
    let payload: { record: AppointmentRecord };
    try {
    ...
    const fcmPayload = {
      message: {
        ...
        notification: {
          ...
          body: `New appointment scheduled with ${patientData.first_name} ${patientData.last_name}`
        },
        data: {
    ...
    let fcmResult: FCMResponse | { raw: string };
