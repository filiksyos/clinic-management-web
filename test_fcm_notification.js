// This is a test script that you can run with Node.js to verify your Firebase Cloud Messaging setup
// It will send a test notification to the 'new_appointments' topic
// Usage: node test_fcm_notification.js
...
// Firebase service account credentials - use your own credentials
...
async function sendTestNotification() {
  try {
    ...
    const fcmPayload = {
      message: {
        ...
        notification: {
          ...
          body: 'This is a test notification from the direct test script'
        },
        data: {
    ...
    const fcmResult = await new Promise((resolve, reject) => {
      const payload = JSON.stringify(fcmPayload);
      ...
      const req = https.request(reqOptions, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            reject(new Error(`Failed to parse FCM response: ${e.message}`));
          }
        });
      });
