...
const getAllFromDB = async (filters: any, options: IPaginationOptions) => {
  try {
      ...
      andConditions.push({
        AND: Object.keys(filterData).map(key => {
          if (key === 'date') {
            ...
            return {
              doctorId: filterData[key],
            };
          } else if (key === 'patientId') {
            return {
              patientId: filterData[key],
            };
          } else if (key === 'scheduleId') {
            return {
              scheduleId: filterData[key],
            };
          } else {
            return {
              [key]: {
    ...
    // Return an empty result set when there's an error
...
const getByIdFromDB = async (id: string) => {
  try {
    ...
    console.error("Error in getByIdFromDB:", error);
    return null;
  }
};
